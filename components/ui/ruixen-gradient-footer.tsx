...
// Its content reads first; the blurred gradient is pinned to the bottom of the
...
//
// Source: 21st.dev (Ruixen). Gradient design inspired by Dia Browser.
// Integrated for Waypoint: the default stops are replaced at the call site
// with the brand's beacon-spectrum (see SiteFooter); the band is aria-hidden,
// pointer-events: none, and scaleY (transform-only) so it can never trap or
// hide content. NOTE: its `position: fixed` band requires NO transformed or
// filtered ancestor around the footer (see the compositor-bug history).
...
// Upstream default stops, floor (0) → top (1).
const RUIXEN_STOPS: GradientStop[] = [
...
export function RuixenGradientFooter({
  ...
  return (
    ...
    >
      ...
      {/* Pinned band: fixed to the viewport — a transformed/filtered ancestor
