  ...
  return (
    <form action={action} className="flex flex-col gap-3">
      ...
      <textarea
        name="body"
        rows={3}
        placeholder="Add a comment for the Northport team…"
  ...
  if (alreadyRequested) {
    return (
      <p className="font-mono text-mono-xs uppercase text-portal-ink-2">
        Remediation {status === "pending_verification" ? "pending Northport verification" : status}.
      </p>
    );
  }
  return (
    <form action={action} className="flex flex-col gap-2">
      ...
      <p className="font-mono text-mono-xs uppercase text-portal-ink-2">
        Sets status to “pending verification” — Northport confirms before it closes.
      </p>
      {state.error && <p className="text-small text-[#e0a06a]">{state.error}</p>}
    </form>
  );
